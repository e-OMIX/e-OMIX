...
import {
  toSignal
} from "./chunk-RRZBIBBN.js";
import {
  SharedResizeObserver
} from "./chunk-G3O7AQTP.js";
...
import {
  ContentObserver
} from "./chunk-Z3QS4VDI.js";
import "./chunk-ZCPTMQPA.js";
import "./chunk-FQL6O3MQ.js";
import "./chunk-UJA2PO5W.js";
import {
  coerceElement
} from "./chunk-O5SOX6PJ.js";
import {
  Platform
} from "./chunk-WKY562PQ.js";
import {
  Directionality
} from "./chunk-IFSIVQEV.js";
import {
  isPlatformBrowser
...
var _SmoothScrollManager = class _SmoothScrollManager {
  ...
  getScrollDestroyerRef(el) {
    ...
    return this.onGoingScrolls.set(el, new import_rxjs.Subject()).get(el);
  }
  /**
  ...
  scrolling(context, destroyed) {
    return this.step(context).pipe(
      ...
      (0, import_rxjs.takeWhile)(() => !this.isReached(context, destroyed)),
      (0, import_rxjs.switchMap)(() => this.scrolling(context, destroyed))
    );
  }
  /**
    ...
    return new Promise((resolve) => {
      this.zone.runOutsideAngular(() => {
        ...
        this.scrolling(context, destroyed).pipe(
          ...
          (0, import_rxjs.takeUntil)(this.interrupted(el, destroyed)),
          // Once finished, clean up the destroyer stream and resolve the promise
          (0, import_rxjs.finalize)(() => this.onScrollReached(el, resolve, destroyed))
        ).subscribe();
      });
    });
  }
...
function preventSelection(doc) {
  return (0, import_rxjs2.tap)(() => doc.onselectstart = () => false);
}
function enableSelection(doc) {
  return (0, import_rxjs2.tap)(() => doc.onselectstart = null);
}
function stopPropagation() {
  return (0, import_rxjs2.tap)((e) => {
    e.preventDefault();
    e.stopPropagation();
  });
}
function getThrottledStream(stream, duration) {
  return stream.pipe((0, import_rxjs2.throttleTime)(duration || 0, null, {
    leading: false,
    trailing: true
  }));
}
        ...
        untracked(() => {
          if (viewportInit) {
            if (disableSensor) {
              ...
              this.zone.runOutsideAngular(() => {
                resizeSub$ = getThrottledStream((0, import_rxjs2.combineLatest)([this.sharedResizeObserver.observe(this.viewport.nativeElement), this.sharedResizeObserver.observe(this.viewport.contentWrapperElement)]), throttleDuration).subscribe(() => {
                  this.zone.run(() => {
...
var _TrackAdapter = class _TrackAdapter extends PointerEventsAdapter {
  ...
  get pointerEvents() {
    const pointerDown$ = (0, import_rxjs2.fromEvent)(this.nativeElement, "pointerdown").pipe(stopPropagation(), preventSelection(this.document));
    const pointerUp$ = (0, import_rxjs2.fromEvent)(this.document, "pointerup", {
      passive: true
    }).pipe(enableSelection(this.document));
    const pointerEnter$ = (0, import_rxjs2.fromEvent)(this.nativeElement, "pointerover", {
      passive: true
    }).pipe(
      // When mouse is out and enters again, must set the current position first
      (0, import_rxjs2.tap)((e) => this.currMousePosition = e[this.control.offsetProperty]),
      (0, import_rxjs2.map)(() => true)
    );
    const pointerLeave$ = (0, import_rxjs2.fromEvent)(this.nativeElement, "pointerout", {
      passive: true
    }).pipe((0, import_rxjs2.map)(() => false));
    const pointerOver$ = (0, import_rxjs2.merge)(pointerEnter$, pointerLeave$).pipe((0, import_rxjs2.startWith)(true));
    const pointerMove$ = (0, import_rxjs2.fromEvent)(this.nativeElement, "pointermove", {
      passive: true
    }).pipe((0, import_rxjs2.tap)((e) => this.currMousePosition = e[this.control.offsetProperty]));
    return pointerDown$.pipe((0, import_rxjs2.switchMap)((startEvent) => {
      pointerMove$.pipe((0, import_rxjs2.takeUntil)(pointerUp$)).subscribe();
      return this.onTrackFirstClick(startEvent).pipe((0, import_rxjs2.delay)(200), (0, import_rxjs2.switchMap)(() => {
        return pointerOver$.pipe((0, import_rxjs2.switchMap)((over) => {
          const currDirection = this.getScrollDirection(this.currMousePosition);
          const sameDirection = this.scrollDirection === currDirection;
          return over && sameDirection ? this.onTrackOngoingMousedown() : import_rxjs2.EMPTY;
        }));
      }), (0, import_rxjs2.takeUntil)(pointerUp$));
    }));
  }
  constructor() {
    afterRenderEffect({
  ...
  onTrackOngoingMousedown() {
    ...
    return this.scrollTo(position).pipe((0, import_rxjs2.takeWhile)(() => !this.isReached(position)), (0, import_rxjs2.switchMap)(() => this.onTrackOngoingMousedown()));
  }
  /**
...
var _TrackXDirective = class _TrackXDirective extends TrackAdapter {
  ...
  scrollTo(start) {
    return (0, import_rxjs2.from)(this.cmp.scrollTo({
      start,
...
var _ThumbAdapter = class _ThumbAdapter extends PointerEventsAdapter {
   ...
   */
  get pointerEvents() {
    return (0, import_rxjs2.fromEvent)(this.nativeElement, "pointerdown").pipe(stopPropagation(), preventSelection(this.document), (0, import_rxjs2.switchMap)((e) => {
      let startTrackMax;
      let startScrollMax;
      const dragStart = (0, import_rxjs2.of)(e).pipe((0, import_rxjs2.tap)(() => {
        startTrackMax = this.trackMax;
        startScrollMax = this.control.viewportScrollMax;
        this.setDragging(this.control.axis);
      }));
      const dragging = (0, import_rxjs2.fromEvent)(this.document, "pointermove").pipe(stopPropagation());
      const dragEnd = (0, import_rxjs2.fromEvent)(this.document, "pointerup", {
        capture: true
      }).pipe(stopPropagation(), enableSelection(this.document), (0, import_rxjs2.tap)(() => this.setDragging("none")));
      return dragStart.pipe((0, import_rxjs2.map)((startEvent) => startEvent[this.control.offsetProperty]), (0, import_rxjs2.switchMap)((startOffset) => dragging.pipe(
        (0, import_rxjs2.map)((moveEvent) => moveEvent[this.control.clientProperty]),
        // Calculate how far the pointer is from the top/left of the scrollbar (minus the dragOffset).
        (0, import_rxjs2.map)((moveClient) => moveClient - this.track.offset),
        (0, import_rxjs2.map)((trackRelativeOffset) => startScrollMax * (trackRelativeOffset - startOffset) / startTrackMax),
        (0, import_rxjs2.tap)((scrollPosition) => this.control.instantScrollTo(scrollPosition, startScrollMax)),
...
var _ScrollbarButton = class _ScrollbarButton extends PointerEventsAdapter {
  get pointerEvents() {
    ...
    const pointerLeave$ = (0, import_rxjs2.fromEvent)(this.nativeElement, "pointerleave", {
      passive: true
    });
    const pointerUpOrLeave$ = (0, import_rxjs2.merge)(pointerUp$, pointerLeave$);
    return pointerDown$.pipe((0, import_rxjs2.switchMap)(() => this.firstScrollStep().pipe((0, import_rxjs2.delay)(this.afterFirstClickDelay), (0, import_rxjs2.switchMap)(() => this.onOngoingPointerdown()), (0, import_rxjs2.takeUntil)(pointerUpOrLeave$))));
  }
  constructor() {
    effect(() => {
  ...
  onOngoingPointerdown() {
    return (0, import_rxjs2.interval)(0, import_rxjs2.animationFrameScheduler).pipe((0, import_rxjs2.takeWhile)(() => this.canScroll(this.control.viewportScrollOffset, this.control.viewportScrollMax)), (0, import_rxjs2.tap)(() => this.onGoingScrollStep()));
  }
};
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      untracked(() => {
        ...
        this.zone.runOutsideAngular(() => {
          sub$ = this.contentObserver.observe(this.scrollbar.nativeElement).pipe((0, import_rxjs2.throttleTime)(100, null, {
            leading: true,
            trailing: true
          })).subscribe(() => {
    ...
    effect((onCleanup) => {
      ...
      untracked(() => {
        if (!disableSensor && contentWrapperElement && spacerElement) {
          this.zone.runOutsideAngular(() => {
            sub$ = getThrottledStream(this.sharedResizeObserver.observe(contentWrapperElement), throttleDuration).pipe((0, import_rxjs2.map)((entries) => filterResizeEntries(entries, contentWrapperElement))).subscribe(() => {
              this.zone.run(() => {
                requestAnimationFrame(() => {
